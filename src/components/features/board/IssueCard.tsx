...
export const IssueCardContent: React.FC<
  IssueCardProps & { isDragging?: boolean }
> = ({ issue, isDragging }) => {
  ...
  const handleCloseIssue = () => {
    updateIssueMutation.mutate({ issueId: issue.id, payload: { status: 'CLOSED' } });
  ...
  return (
    ...
    >
      ...
      <h4
        className={`text-sm font-medium mb-1 ${isDone ? "text-muted-foreground" : "text-foreground"}`}
      >
        {issue.title}
      </h4>
        ...
        <DropdownMenu>
          ...
          >
            ...
            <DropdownMenuItem 
              ...
              onClick={(e) => { e.stopPropagation(); setIsCloseDialogOpen(true); }}
            >
              Cerrar tarjeta
            </DropdownMenuItem>
          </DropdownMenuContent>
      ...
      <ConfirmationDialog
        ...
        description="¿Está seguro que desea cerrar esta tarjeta? Esta acción la quitará del tablero."
